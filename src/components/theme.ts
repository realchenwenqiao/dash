 ...
 * # DASH 色卡 (color card)
 *
 * One indigo→azure brand family (the DeepSeek gradient) plus a neutral pair
 * and three conventional status colors.
 *
 * Brand family (`#4D6BFE → #3982FF → #2498FF`, the deepseek.com logo stops):
 * | role   | 16-color | truecolor   | where it appears                              |
 * |--------|----------|-------------|-----------------------------------------------|
 * | logo   | —        | full gradient| the DASH ASCII logo (`gradientText`)           |
 * | brand  | 34 blue  | #4D6BFE     | brand art fallback                            |
 * | accent | 94 bright-blue | #3982FF | headings, prompt, slash commands, selection, quote borders |
 * | code   | 36 cyan  | #2498FF     | inline code, code blocks                      |
 *
 * Background (message blocks): a full-width dark-indigo fill (`#1E2A5A`) behind
 * user messages, thinking, and notes, painted by {@link quoteBlockBackground}
 * and applied through Markdown's default background so it spans the whole line.
 *
 * Neutral:
 * | role | escape | where it appears                        |
 * |------|--------|-----------------------------------------|
 * | text | (none) | body text, the terminal default         |
 * | dim  | SGR 2  | tool bodies, chrome, footers, reasoning |
 *
 * Status (kept conventional on purpose — a diff or an error must read
 * instantly, and blue cannot mean both “added” and “removed”):
 * | role    | 16-color | where it appears         |
 * |---------|----------|--------------------------|
 * | success | 32 green | succeeded calls, diff +  |
 * | warning | 33 yellow| pending calls, warnings  |
 * | error   | 31 red   | failures, diff −         |
 * @module @realchenwenqiao/dash/components/theme
 */
...
export function paletteSpec(scheme: TerminalColorScheme): {
  ...
  return {
    colors: {
      ...
      accent: { open: '94', close: '39', purpose: 'The one emphasis color (brand azure): headers, prompt, slash commands, selection' },
...
/**
 * Full-line dark-indigo block background for message fills. Used as the
 * Markdown default background so the fill spans the entire row (content plus
 * trailing padding), not just the text.
 * @param enabled - whether ANSI is emitted at all.
 * @returns a wrapper that paints one full line, or a pass-through when disabled.
 */
export function quoteBlockBackground(enabled: boolean): (line: string) => string {
  if (!enabled) return line => line
  const truecolor = process.env.COLORTERM === 'truecolor'
  const open = truecolor ? '\x1b[48;2;30;42;90;97m' : '\x1b[100;97m'
  const close = '\x1b[39;49m'
...
export function markdownTheme(palette: Palette): MarkdownTheme {
  return {
    ...
    quote: (text) => {
      // The full-line background comes from defaultTextStyle.bgColor; here we
      // only keep error/limit labels semantic red inside the block.
      if (text.includes('[error:') || text.includes('[max-tokens]')) return palette.error(text)
      return text
    },
